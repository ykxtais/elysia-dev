...
namespace ElysiaAPI.Controllers
{
    ...
    public class MotoController : ControllerBase
    {
        ...
        private MotoResponse WithLinks(MotoResponse r)
        {
            ...
            r.Links.Clear();
            r.Links.Add(new Link("self",   Url.Link(nameof(GetMoto),    new { id = r.Id, version })!));
            r.Links.Add(new Link("update", Url.Link(nameof(UpdateMoto), new { id = r.Id, version })!, "PUT"));
            r.Links.Add(new Link("delete", Url.Link(nameof(DeleteMoto), new { id = r.Id, version })!, "DELETE"));
            r.Links.Add(new Link("list",   Url.Link(nameof(GetMotos),   new { page = 1, pageSize = 10, version })!));
            return r;
        }
        ...
        private List<Link> CollectionLinks(string routeName, int page, int pageSize, int total, object? extra = null)
        {
            ...
            object Params(int p) => new { page = p, pageSize, version };
        ...
        public async Task<ActionResult<MotoResponse>> CreateMoto([FromBody] MotoRequest request)
        {
            try
            {
                ...
                return CreatedAtAction(nameof(GetMoto), new { id = moto.Id, version }, resp);
            }
