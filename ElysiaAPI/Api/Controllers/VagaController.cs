...
namespace ElysiaAPI.Controllers
{
    ...
    public class VagaController : ControllerBase
    {
        ...
        private VagaResponse WithLinks(VagaResponse r)
        {
            ...
            r.Links.Clear();
            r.Links.Add(new Link("self",   Url.Link(nameof(GetVaga),    new { id = r.Id, version })!));
            r.Links.Add(new Link("update", Url.Link(nameof(UpdateVaga), new { id = r.Id, version })!, "PUT"));
            r.Links.Add(new Link("delete", Url.Link(nameof(DeleteVaga), new { id = r.Id, version })!, "DELETE"));

            if (string.Equals(r.Status, "Livre", StringComparison.OrdinalIgnoreCase))
                r.Links.Add(new Link("ocupar", Url.Link(nameof(UpdateVaga), new { id = r.Id, version })!, "PUT"));
            if (string.Equals(r.Status, "Ocupada", StringComparison.OrdinalIgnoreCase))
                r.Links.Add(new Link("liberar", Url.Link(nameof(UpdateVaga), new { id = r.Id, version })!, "PUT"));
        ...
        private List<Link> CollectionLinks(string routeName, int page, int pageSize, int total, string? patio = null)
        {
            ...
            object Params(int p)
                => patio is null
                    ? new { page = p, pageSize, version }
                    : new { patio, page = p, pageSize, version };
        ...
        public async Task<ActionResult<VagaResponse>> CreateVaga([FromBody] VagaRequest request)
        {
            try
            {
                ...
                return CreatedAtAction(nameof(GetVaga), new { id = vaga.Id, version }, resp);
            }
